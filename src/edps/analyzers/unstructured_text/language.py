...
def calculate_language_confidences(text: str) -> DataFrame:
...
def extract_languages(ctx: TaskContext, confidences: DataFrame) -> Set[str]:
    ...
    languages = set(confidences.drop(columns=_TEXT_BODY_KEY).idxmax(axis=1))
...
def detect_word_cloud(ctx: TaskContext, confidences: DataFrame, top_n: int = 10) -> Iterator[WordFrequency]:
    ...
    best_language_per_sentence = confidences.drop(columns=_TEXT_BODY_KEY).idxmax(axis=1)
    text_by_lang = confidences.groupby(best_language_per_sentence)[_TEXT_BODY_KEY].apply(". ".join)
